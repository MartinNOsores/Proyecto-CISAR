...
def ingreso_exitoso(update: Update, context: CallbackContext, nombreuser) -> None:
    htext = '''Bienvenido/a ''' + nombreuser + '''\n\nPuede ingresar a la cabina\n\n'''
    Htext = f"Fecha y Hora de ingreso: \n\n{fecha_hora}"
    update.message.reply_text(htext + Htext)

def ingreso_sospechoso(update: Update, context: CallbackContext) -> None:
    htext = ''' Debe volver a su hogar'''
...
def chequearUsuarios(update, context): 
    ...
    if check > 0:
        ...
        if (GPIO.input(pulsador) == 1):
            r = requests.get("https://api.telegram.org/bot1611398547:AAG9YCiIxoW1SrGpsSHzDj1vSXMlqLf5kEY/sendMessage?chat_id=-1001507958281&text=El%20sujeto%20" + nombreuser + "%20presenta%20%20sintomas%0A%0ACurso: " + cursouser + "%0A%0ADivision: " +  divisionuser + "%0A%0AEspecialidad: " + especialidaduser + "%0A%0ADNI:" + dniuser + "%0A%0A/Voy"  )
            with open("index.html", "wb") as f:   
                f.write(r.content)
                r.close()
            print ("Pulsador encendido")
            print ("$$$$$$$$$$$$$$$$$$")
            time.sleep(1)
            ingreso_sospechoso(update, context)
        
        else:
            print ("Pulsador apagado")
            print ("$$$$$$$$$$$$$$$$$$")
            time.sleep(1)
